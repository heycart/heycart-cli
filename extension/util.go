...
func PlatformPath(projectRoot, component, path string) string {
	...
	} else if _, err := os.Stat(filepath.Join(projectRoot, "vendor", "heycart", "platform")); err == nil {
		return filepath.Join(projectRoot, "vendor", "heycart", "platform", "src", component, path)
	}

	return filepath.Join(projectRoot, "vendor", "heycart", strings.ToLower(component), path)
}

// IsContributeProject checks if the project is a contribution project aka heycart/heycart.
